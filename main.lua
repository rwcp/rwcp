-- m0pu
-- Production-grade Luau analysis and source-recovery pipeline.
...
-- ============================================================================
-- m0pu
-- A deterministic Luau middle-end.
--
-- Design:
--   serialized bytecode
...
local function irreducibleRegions(g,dom)
    ...
    local function visit(v)
        idx+=1; index[v]=idx; low={[v]=idx}; stack[#stack+1]=v; on[v]=true
    ...
    for _,comp in ipairs(sccs) do
        if #comp>1 then
            ...
            local n=0; for _ in pairs(entries) do n+=1 end
...
function Structurer:walk(id,stop,depth,active,pr)
    ...
    if loop and not self.seen["loop"..id] then
        ...
        elseif shape.kind=="generic" and prep then
            ...
            if li and li.name=="FORGLOOP" then count=(li.aux and (li.aux.raw&0xff)) or 1 end
            local vars={}
...
function m0pu.runRegression(options)
    ...
    for _,case in ipairs(REGRESSION_CASES) do
        ...
        if not v.available then
            results.skipped+=1; results.cases[#results.cases+1]={name=case.name,status="skipped",reason=v.reason}
        elseif not v.ok then
            results.failed+=1; results.cases[#results.cases+1]={name=case.name,status="compile_failed",error=v.error}
        else
            ...
            if ok then
                ...
                results.failed+=1; results.cases[#results.cases+1]={name=case.name,status="decompile_failed",error=tostring(r)}
            end
        end
    end
    return results
...
local function semanticCoverage(p)
    ...
    for _ in pairs(unknown) do known+=1 end
