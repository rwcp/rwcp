...
-- ============================================================================
-- m0pu v4 Production Recovery Layer
--
-- This is the decompiler's compiler-like middle end.  The important design
-- rule is: bytecode is never turned directly into source.  It is lifted into
-- facts (CFG/SSA/value graph), optimized, structured into regions, and only
-- then printed.  This makes the emitter replaceable and keeps recovery logic
-- independent of syntax formatting.
-- ============================================================================

m0pu.VERSION = "4.0-PRO"
m0pu.features = {
    "versioned-luau-loader", "normalized-ir", "cfg", "dominators",
    "postdominators", "dominance-frontiers", "true-ssa", "phi-placement",
    "use-def", "value-graph", "copy-propagation", "constant-folding",
    "branch-simplification", "dead-value-analysis", "natural-loops",
    "control-dependence", "region-recovery", "if-else-recovery",
    "while-repeat-recovery", "numeric-for-recovery", "generic-for-recovery",
    "short-circuit-recovery", "closure-recovery", "debug-name-recovery",
    "deterministic-emission", "confidence-scoring", "validation-diagnostics",
...
-- ----------------------- Dominance frontiers -------------------------------
local function computeDomFrontiers(g,idom)
    local df={}
...
-- ----------------------------- SSA -----------------------------------------
-- Classic Cytron-style phi placement + dominator-tree renaming, adapted to
-- Luau's register machine.  Each bytecode register is treated as a mutable
-- variable; every write creates a new SSA value.
local ProdSSA={}; ProdSSA.__index=ProdSSA

function ProdSSA.new(p,g,dom,idom,domChildren,options)
    return setmetatable({
        p=p,g=g,dom=dom,idom=idom,children=domChildren,options=options or {},
        df=computeDomFrontiers(g,idom),
        phiBlocks={},phis={},values={},nextValue=0,uses={},
        inState={},outState={},inst={},iterations=1,
        defsByReg={},unknownByReg={},
    },ProdSSA)
end

function ProdSSA:newValue(kind,data)
    self.nextValue=self.nextValue+1
    local v={id=self.nextValue,kind=kind,data=data or {},uses={},def=nil}
    self.values[v.id]=v
    return v
end

function ProdSSA:addUse(v,site)
    if v then v.uses[#v.uses+1]=site end
end

function ProdSSA:const(text) return self:newValue("const",{text=text}) end
function ProdSSA:unknown(reg)
    if not self.unknownByReg[reg] then self.unknownByReg[reg]=self:newValue("unknown",{reg=reg}) end
    return self.unknownByReg[reg]
end
function ProdSSA:op(op,args,block,pc,extra)
    local v=self:newValue("op",{op=op,args=args or {},block=block,pc=pc,extra=extra})
    for _,a in ipairs(args or {}) do self:addUse(a,{block=block,pc=pc,role="operand"}) end
    return v
end

local function writesReg(i)
    local n=i.name
    if n=="LOADNIL" or n=="LOADB" or n=="LOADN" or n=="LOADK" or n=="LOADKX" or n=="MOVE" or
       n=="GETGLOBAL" or n=="GETUPVAL" or n=="GETIMPORT" or n=="GETTABLE" or n=="GETTABLEKS" or
       n=="GETUDATAKS" or n=="GETTABLEN" or n=="NEWTABLE" or n=="DUPTABLE" or n=="NEWCLOSURE" or
       n=="DUPCLOSURE" or n=="NAMECALL" or n=="NAMECALLUDATA" or n=="GETVARARGS" or
       n=="ADD" or n=="SUB" or n=="MUL" or n=="DIV" or n=="MOD" or n=="POW" or n=="IDIV" or
       n=="AND" or n=="OR" or n=="ADDK" or n=="SUBK" or n=="MULK" or n=="DIVK" or n=="MODK" or
       n=="POWK" or n=="IDIVK" or n=="ANDK" or n=="ORK" or n=="SUBRK" or n=="DIVRK" or
       n=="CONCAT" or n=="NOT" or n=="MINUS" or n=="LENGTH" or n=="CALL" or n=="CALLFB" or
       n=="GETUPVAL" then return {i.A} end
    return {}
end

function ProdSSA:collectDefs()
    for _,b in ipairs(self.g.blocks) do
        for _,i in ipairs(b.instructions) do
            for _,r in ipairs(writesReg(i)) do
...
function ProdSSA:placePhis()
    for reg,defs in pairs(self.defsByReg) do
        local has={}; local work={}
        for b in pairs(defs) do work[#work+1]=b end
        local qi=1
        while qi<=#work do
            local x=work[qi]; qi=qi+1
            for y in pairs(self.df[x] or {}) do
                if not has[y] then
                    has[y]=true
                    self.phiBlocks[y]=self.phiBlocks[y] or {}
                    self.phiBlocks[y][reg]=true
                    if not defs[y] then work[#work+1]=y end
                end
            end
        end
    end
    for bid,regs in pairs(self.phiBlocks) do
        self.phis[bid]={}
        for reg in pairs(regs) do
            local v=self:newValue("phi",{reg=reg,block=bid,incomings={}})
            self.phis[bid][reg]=v
        end
    end
end

function ProdSSA:literal(ix)
    local k=(self.p.constants or {})[(ix or 0)+1]
    if not k then return "nil" end
    return literal(k)
end
function ProdSSA:K(ix) return self:const(self:literal(ix)) end

function ProdSSA:lift(i,st,b)
    local n,A,B,C,D=i.name,i.A,i.B,i.C,i.D
    local function V(r)
        local v=st[r] or self:unknown(r)
        return v
    end
    local function DEF(r,v)
        st[r]=v; v.def={block=b.id,pc=i.pc,reg=r}
        self.defsByReg[r]=self.defsByReg[r] or {}
        self.defsByReg[r][b.id]=true
    end
    local v,args
    if n=="LOADNIL" then DEF(A,self:const("nil"))
    elseif n=="LOADB" then DEF(A,self:const(B~=0 and "true" or "false"))
    elseif n=="LOADN" then DEF(A,self:const(tostring(D)))
    elseif n=="LOADK" then DEF(A,self:K(D))
    elseif n=="LOADKX" then DEF(A,self:K(i.aux and i.aux.KV or 0))
    elseif n=="MOVE" then DEF(A,V(B))
    elseif n=="GETGLOBAL" then DEF(A,self:newValue("global",{name=self:K(i.aux and i.aux.KV or 0)}))
    elseif n=="GETUPVAL" then DEF(A,self:newValue("upvalue",{index=B,name=self.p.upvalues and self.p.upvalues[B+1]}))
    elseif n=="GETIMPORT" then
        local k=(self.p.constants or {})[(i.aux and i.aux.KV or 0)+1]
        DEF(A,self:newValue("import",{path=k and k.path or "import"}))
    elseif n=="GETTABLE" then DEF(A,self:op("index",{V(B),V(C)},b.id,i.pc))
    elseif n=="GETTABLEKS" or n=="GETUDATAKS" then DEF(A,self:op("field",{V(B),self:K(i.aux and i.aux.KV or 0)},b.id,i.pc))
    elseif n=="GETTABLEN" then DEF(A,self:op("index",{V(B),self:const(tostring(C+1))},b.id,i.pc))
    elseif n=="NEWTABLE" then DEF(A,self:newValue("table",{block=b.id,pc=i.pc}))
    elseif n=="DUPTABLE" then DEF(A,self:op("duptable",{self:K(D)},b.id,i.pc))
    elseif n=="NEWCLOSURE" or n=="DUPCLOSURE" then DEF(A,self:newValue("closure",{proto=i.aux and i.aux.KV}))
    elseif n=="NAMECALL" or n=="NAMECALLUDATA" then DEF(A,self:op("methodcall",{V(B),self:K(i.aux and i.aux.KV or 0)},b.id,i.pc))
    elseif n=="CALL" or n=="CALLFB" then
        args={V(A)}
        if B>1 then for r=A+1,A+B-1 do args[#args+1]=V(r) end end
        local results=(C==0 and 1 or math.max(1,C-1))
        for r=0,results-1 do DEF(A+r,self:op("call",args,b.id,i.pc,{multi=(results>1)})) end
    elseif n=="GETVARARGS" then DEF(A,self:newValue("varargs",{block=b.id,pc=i.pc}))
    elseif n=="CONCAT" then
        args={}; for r=B,C do args[#args+1]=V(r) end; DEF(A,self:op("concat",args,b.id,i.pc))
    ...
    elseif n=="ADDK" or n=="SUBK" or n=="MULK" or n=="DIVK" or n=="MODK" or n=="POWK" or n=="IDIVK" or n=="ANDK" or n=="ORK" then
        DEF(A,self:op(n:sub(1,-2):lower(),{V(B),self:K(C)},b.id,i.pc))
    elseif n=="SUBRK" or n=="DIVRK" then DEF(A,self:op(n:sub(1,-3):lower(),{self:K(B),V(C)},b.id,i.pc))
    elseif n=="FORNPREP" or n=="FORNLOOP" or n=="FORGPREP" or n=="FORGPREP_INEXT" or n=="FORGPREP_NEXT" or n=="FORGLOOP" then
        self.inst[#self.inst+1]={kind="loopop",pc=i.pc,block=b.id,op=n,A=A,B=B,C=C,D=D,state=deepcopyState(st)}
    elseif n=="SETUPVAL" or n=="SETGLOBAL" or n=="SETTABLE" or n=="SETTABLEKS" or n=="SETTABLEN" then
        self.inst[#self.inst+1]={kind="store",pc=i.pc,block=b.id,op=n,A=A,B=B,C=C,D=D,state=deepcopyState(st)}
    elseif n:match("^JUMP") or n=="CMPPROTO" then
        self.inst[#self.inst+1]={kind="branch",pc=i.pc,block=b.id,op=n,A=A,B=B,C=C,D=D,aux=i.aux,state=deepcopyState(st)}
    elseif n=="RETURN" then
        local vals={}
        if B==0 then vals[1]=self:newValue("vararg_return",{}) else for r=A,A+B-2 do vals[#vals+1]=V(r) end end
        for _,x in ipairs(vals) do self:addUse(x,{block=b.id,pc=i.pc,role="return"}) end
        self.inst[#self.inst+1]={kind="return",pc=i.pc,block=b.id,values=vals,state=deepcopyState(st)}
    else
        self.inst[#self.inst+1]={kind="opaque",pc=i.pc,block=b.id,op=n,A=A,B=B,C=C,D=D,state=deepcopyState(st)}
    end
end

function ProdSSA:renameBlock(bid,parentState)
    local b=self.g.map[bid]
    local st=deepcopyState(parentState)
    for reg,phi in pairs(self.phis[bid] or {}) do st[reg]=phi end
    self.inState[bid]=deepcopyState(st)
    for _,i in ipairs(b.instructions) do self:lift(i,st,b) end
    self.outState[bid]=deepcopyState(st)
    for _,child in ipairs(self.children[bid] or {}) do self:renameBlock(child,st) end
end

function ProdSSA:build()
    self:collectDefs(); self:placePhis()
    self.inState[self.g.entry.id]={}
    self:renameBlock(self.g.entry.id,{})
    -- Materialize phi incoming edges after all predecessor states exist.
    for bid,phis in pairs(self.phis) do
        ...
        for reg,phi in pairs(phis) do
            for _,pred in ipairs(b.predecessors) do
                ...
                self:addUse(v,{block=bid,role="phi",from=pred})
            end
        end
    end
    return self
...
-- -------------------------- Value optimization -----------------------------
local function valueText(v,ssa,p,names,seen)
    seen=seen or {}
    if not v then return "nil",0 end
    if seen[v.id] then return "v"..v.id,0 end
    seen[v.id]=true
    local d=v.data or {}; local k=v.kind
    if k=="const" then return d.text or "nil",100 end
    if k=="unknown" then return names[d.reg] or ("v"..d.reg),5 end
    if k=="upvalue" then return d.name or ("upvalue"..d.index),70 end
    if k=="global" then return valueText(d.name,ssa,p,names,seen),70 end
    if k=="import" then return d.path or "import",90 end
    if k=="varargs" or k=="vararg_return" then return "...",90 end
    if k=="table" then return "{}",80 end
    if k=="closure" then return "function(...) end",40 end
    if k=="phi" then return names[d.reg] or ("v"..v.id),60 end
    if k=="op" then
        local a={}; local conf=100
        for _,x in ipairs(d.args or {}) do local t,c=valueText(x,ssa,p,names,seen); a[#a+1]=t; conf=math.min(conf,c) end
        local op=d.op
        if op=="index" then return a[1].."["..a[2].."]",conf
        if op=="field" then
            local key=a[2] or '"field"'; local raw=key:gsub('^"(.*)"$','%1')
            return identifier(raw) and a[1].."."..raw or a[1].."["..key.."]",conf
        end
        if op=="methodcall" then
            local key=(a[2] or '"method"'):gsub('^"(.*)"$','%1'); return a[1]..":"..key.."(...)",conf
        end
        if op=="call" then return a[1].."(...)",conf end
        if op=="concat" then return table.concat(a," .. "),conf end
        local sym={add="+",sub="-",mul="*",div="/",mod="%",pow="^",idiv="//",["and"]="and",["or"]="or"}
        if sym[op] then return "("..a[1].." "..sym[op].." "..a[2]..")",conf end
        if op=="not" then return "not ("..a[1]..")",conf end
        if op=="neg" then return "-"..a[1],conf end
        if op=="len" then return "#"..a[1],conf end
        if op=="duptable" then return a[1] or "{}",conf end
    end
    return "v"..v.id,0
end

local function foldValue(v,memo)
    if not v or memo[v.id] then return v end
    memo[v.id]=true
    if v.kind~="op" then return v end
    local d=v.data
    for _,a in ipairs(d.args or {}) do foldValue(a,memo) end
    local args=d.args or {}
    if (#args==2) and args[1].kind=="const" and args[2].kind=="const" then
        local a,b=args[1].data.text,args[2].data.text
        local na,nb=tonumber(a),tonumber(b)
        if na and nb then
            local r
            if d.op=="add" then r=na+nb elseif d.op=="sub" then r=na-nb elseif d.op=="mul" then r=na*nb elseif d.op=="div" and nb~=0 then r=na/nb elseif d.op=="mod" and nb~=0 then r=na%nb elseif d.op=="pow" then r=na^nb elseif d.op=="idiv" and nb~=0 then r=math.floor(na/nb) end
            if r~=nil then v.kind="const"; v.data={text=tostring(r)} end
        end
    elseif d.op=="not" and args[1] and args[1].kind=="const" then
        local x=args[1].data.text; if x=="true" then v.kind="const"; v.data={text="false"} elseif x=="false" or x=="nil" then v.kind="const"; v.data={text="true"} end
    end
    return v
end

local function optimizeSSA(ssa)
    local memo={}
    for _,v in pairs(ssa.values) do foldValue(v,memo) end
    -- Copy propagation: MOVE and phi-singleton values are represented by the
    -- same value object already, so no textual substitution pass is required.
    return ssa
end

-- ---------------------------- Region analysis ------------------------------
local function regionPlan(g,pdom,ipdom,loops)
    local regions={blocks={},branches={},loops={},joins={}}
    for _,b in ipairs(g.blocks) do regions.blocks[b.id]={kind="basic",id=b.id,start=b.start,finish=b.finish} end
    for _,l in ipairs(loops) do
        regions.loops[#regions.loops+1]={header=l.header,latch=l.latch,members=l.members,exit=nil}
        local mem={}; for _,x in ipairs(l.members) do mem[x]=true end
        for _,x in ipairs(l.members) do
            for _,s in ipairs(g.map[x].successors) do if not mem[s] then regions.loops[#regions.loops].exit=s; break end end
            if regions.loops[#regions.loops].exit then break end
        end
    end
    for _,b in ipairs(g.blocks) do
        if #b.successors==2 then
            regions.branches[#regions.branches+1]={header=b.id,left=b.successors[1],right=b.successors[2],join=ipdom[b.id]}
        end
    end
    return regions
end

-- ----------------------- Production source structurer ----------------------
local ProductionStructurer={}; ProductionStructurer.__index=ProductionStructurer
function ProductionStructurer.new(p,g,dom,idom,pdom,ipdom,loops,ssa,options)
    return setmetatable({p=p,g=g,dom=dom,idom=idom,pdom=pdom,ipdom=ipdom,loops=loops,ssa=ssa,opt=options or {},regions=regionPlan(g,pdom,ipdom,loops),seen={},declared={},confidence=0,warnings={}},ProductionStructurer)
end
function ProductionStructurer:names()
    local n={}
    for _,l in ipairs(self.p.locals or {}) do if l.name and l.name~="" then n[l.reg]=l.name end end
    for r=0,(self.p.maxstack or 1)-1 do n[r]=n[r] or ("v"..r) end
    return n
end
function ProductionStructurer:expr(v)
    return valueText(v,self.ssa,self.p,self:names())
end
function ProductionStructurer:cond(ins,st)
    local function R(r) return self:expr(st[r]) end
    local n=ins.name
    local cmp={JUMPIFEQ="==",JUMPIFLE="<=",JUMPIFLT="<",JUMPIFNOTEQ="~=",JUMPIFNOTLE=">",JUMPIFNOTLT=">="}
    if n=="JUMPIF" then return R(ins.A),true end
    if n=="JUMPIFNOT" then return "not ("..R(ins.A)..")",true end
    if cmp[n] then return "("..R(ins.A).." "..cmp[n].." "..R(ins.C)..")",true end
    if n=="JUMPXEQKNIL" then return "("..R(ins.A).." "..((ins.aux and ins.aux.NOT) and "~=" or "==").." nil)",true end
    if n=="JUMPXEQKB" then return "("..R(ins.A).." "..((ins.aux and ins.aux.NOT) and "~=" or "==").." "..((ins.aux and ins.aux.KB) and "true" or "false")..")",true end
    return "true",false
end
function ProductionStructurer:emitInst(b,i,depth)
    local n,A,B,C=i.name,i.A,i.B,i.C; local st=self.ssa.outState[b.id] or {}; local names=self:names()
    local function R(r) return self:expr(st[r]) end
    local function L(r) return names[r] or ("v"..r) end
    local function decl(r,txt)
        if not self.declared[r] then self.declared[r]=true; return "local "..L(r).." = "..txt end
        return L(r).." = "..txt
    end
    if n=="LOADNIL" or n=="LOADB" or n=="LOADN" or n=="LOADK" or n=="LOADKX" or n=="MOVE" or n=="GETUPVAL" or n=="GETGLOBAL" or n=="GETIMPORT" or n=="GETTABLE" or n=="GETTABLEKS" or n=="GETUDATAKS" or n=="GETTABLEN" or n=="ADD" or n=="SUB" or n=="MUL" or n=="DIV" or n=="MOD" or n=="POW" or n=="IDIV" or n=="AND" or n=="OR" or n=="ADDK" or n=="SUBK" or n=="MULK" or n=="DIVK" or n=="MODK" or n=="POWK" or n=="IDIVK" or n=="ANDK" or n=="ORK" or n=="SUBRK" or n=="DIVRK" or n=="CONCAT" or n=="NOT" or n=="MINUS" or n=="LENGTH" then
        return decl(A,R(A))
    end
    if n=="NEWTABLE" or n=="DUPTABLE" or n=="NEWCLOSURE" or n=="DUPCLOSURE" or n=="NAMECALL" or n=="NAMECALLUDATA" then return decl(A,R(A)) end
    if n=="SETUPVAL" then return (self.p.upvalues and self.p.upvalues[B+1] or "upvalue"..B).." = "..R(A) end
    if n=="SETGLOBAL" then
        local k=(self.p.constants[(i.aux and i.aux.KV or 0)+1] or {}).value or "global"; return identifier(k) and k.." = "..R(A) or "_G["..quote(k).."] = "..R(A)
    end
    if n=="SETTABLE" then return R(A).."["..R(B).."] = "..R(C) end
    if n=="SETTABLEN" then return R(A).."["..(C+1).."] = "..R(B) end
    if n=="SETTABLEKS" or n=="SETUDATAKS" then
        local k=(self.p.constants[(i.aux and i.aux.KV or 0)+1] or {}).value or "field"; return identifier(k) and R(A).."."..k.." = "..R(B) or R(A).."["..quote(k).."] = "..R(B)
    end
    if n=="CALL" or n=="CALLFB" then return R(A) end
    if n=="RETURN" then
        if B==0 then return "return ..." end
        local q={}; for r=A,A+B-2 do q[#q+1]=R(r) end; return "return "..table.concat(q,", ")
    end
    if n=="GETVARARGS" then return decl(A,R(A)) end
    if n=="CAPTURE" or n=="PREPVARARGS" or n=="COVERAGE" or n=="FASTCALL" or n=="FASTCALL1" or n=="FASTCALL2" or n=="FASTCALL2K" or n=="FASTCALL3" or n=="NATIVECALL" then return nil end
    if n:match("^JUMP") or n=="CMPPROTO" or n:match("^FOR") then return nil end
    return "-- unsupported "..n
end

function ProductionStructurer:loopAt(id)
    for _,l in ipairs(self.loops) do if l.header==id then return l end end
end
function ProductionStructurer:branch(b)
    local i=b.instructions[#b.instructions]; if not i then return nil end
    local n=i.name; if not (n=="JUMPIF" or n=="JUMPIFNOT" or n:match("^JUMPIF") or n:match("^JUMPXEQ")) then return nil end
    if #b.successors~=2 then return nil end
    local targetPc=target(i.pc,i); local targetBlock=self.g.byPc[targetPc]; local fall=self.g.byPc[i.pc+oplen(n)]
    if not targetBlock or not fall then return nil end
    return {instr=i,target=targetBlock.id,fall=fall.id,join=self.ipdom[b.id]}
end
function ProductionStructurer:recover()
    local out={}
    local function put(d,s) if s and s~="" then out[#out+1]=string.rep("    ",d)..s end end
    local active={}
    local function walk(id,depth,stop)
        if id==stop or active[id] then return end
        local b=self.g.map[id]; if not b then return end
        active[id]=true
        local loop=self:loopAt(id)
        if loop and not self.seen["L"..id] then
            self.seen["L"..id]=true
            local exit=nil; local mem={}; for _,x in ipairs(loop.members) do mem[x]=true end
            for _,x in ipairs(loop.members) do for _,s in ipairs(self.g.map[x].successors) do if not mem[s] then exit=s; break end end if exit then break end end
            local term=b.instructions[#b.instructions]
            if term and (term.name=="JUMPIF" or term.name=="JUMPIFNOT" or term.name:match("^JUMPIF") or term.name:match("^JUMPXEQ")) then
                local bi=self:branch(b); local st=self.ssa.outState[id] or {}; local c,ok=self:cond(bi.instr,st)
                if ok and bi then
                    if bi.target==exit then put(depth,"while not ("..c..") do") else put(depth,"while "..c.." do") end
                else put(depth,"while true do") end
            elseif term and term.name=="FORNPREP" then put(depth,"for v"..term.A.." = v"..term.A..", v"..(term.A+1).." do")
            elseif term and (term.name=="FORGPREP" or term.name=="FORGPREP_INEXT" or term.name=="FORGPREP_NEXT") then put(depth,"for v"..(term.A+3).." in v"..term.A.." do")
            else put(depth,"while true do") end
            for _,s in ipairs(b.successors) do if mem[s] and s~=id then walk(s,depth+1,id) end end
            put(depth,"end")
            if exit then walk(exit,depth,stop) end
            active[id]=nil; return
        end
        local br=self:branch(b)
        if br and br.join and br.join~=id and not active[br.join] then
            local st=self.ssa.outState[id] or {}; local c,ok=self:cond(br.instr,st)
            if ok then
                -- Orient the condition so the branch that actually executes
                -- when the predicate is true is emitted first.
                local first,second=br.target,br.fall
                local neg=false
                if br.instr.name=="JUMPIFNOT" or br.instr.name:match("JUMPIFNOT") then neg=true end
                if neg then first,second=second,first end
                put(depth,"if "..c.." then")
                walk(first,depth+1,br.join)
                if second~=br.join then put(depth,"else"); walk(second,depth+1,br.join) end
                put(depth,"end")
                self.confidence+=4
                walk(br.join,depth,stop)
                active[id]=nil; return
            end
        end
        if not self.seen["B"..id] then
            self.seen["B"..id]=true
            for _,i in ipairs(b.instructions) do put(depth,self:emitInst(b,i,depth)) end
        end
        if #b.successors==1 then walk(b.successors[1],depth,stop)
        elseif #b.successors>1 then
            -- Irreducible/ambiguous control flow: preserve correctness with a
            -- labelled fallback rather than inventing structured semantics.
            self.warnings[#self.warnings+1]="unstructured control flow at block "..id
            for _,s in ipairs(b.successors) do if s~=stop then put(depth,"-- goto B"..s) end end
        end
        active[id]=nil
    end
    walk(self.g.entry.id,0,nil)
    return table.concat(out,"\n")
end

-- --------------------------- Diagnostics -----------------------------------
local function productionConfidence(g,ssa,regions,structurer)
    local score,max=0,0
    max=#g.blocks*4 + math.max(1,ssa.nextValue) + #regions.branches*2 + #regions.loops*3
    for _,b in ipairs(g.blocks) do
        if #b.successors<=2 then score+=2 end
        if #b.predecessors<=2 then score+=1 end
    end
    score+=#regions.branches*2 + #regions.loops*3 + structurer.confidence
    for _,v in pairs(ssa.values) do if v.kind=="phi" then score+=1 end end
    local pct=math.floor(math.min(100,(score/math.max(1,max))*100)+0.5)
    return {score=score,maximum=max,percent=pct,structured=structurer.confidence,warnings=#structurer.warnings}
end

local function productionDiagnostics(p,g,ssa,regions,structurer)
    local d={errors={},warnings={},facts={}}
    local reachable={}; local q={g.entry.id}; local qi=1
    while qi<=#q do local x=q[qi]; qi+=1; if not reachable[x] then reachable[x]=true; for _,s in ipairs(g.map[x].successors) do q[#q+1]=s end end end
    for _,b in ipairs(g.blocks) do if not reachable[b.id] then d.warnings[#d.warnings+1]="unreachable block B"..b.id end end
    for bid,phis in pairs(ssa.phis) do
        for reg,phi in pairs(phis) do if #phi.data.incomings<2 then d.warnings[#d.warnings+1]=("degenerate phi at B%d:R%d"):format(bid,reg) end end
    end
    for _,w in ipairs(structurer.warnings) do d.warnings[#d.warnings+1]=w end
    d.facts={blocks=#g.blocks,edges=0,ssaValues=ssa.nextValue,phis=0,branches=#regions.branches,loops=#regions.loops}
    for _,b in ipairs(g.blocks) do d.facts.edges+=#b.successors end
    for _,x in pairs(ssa.phis) do for _ in pairs(x) do d.facts.phis+=1 end end
    return d
end

-- ----------------------- Production API override ---------------------------
local function productionOne(p,options,chunk)
    local g=advCFG(p)
    local dom,idom,children=computeDominators(g)
    local pdom,ipdom,exits=computePostDominators(g)
    local loops=findLoops(g,dom)
    local ssa=ProdSSA.new(p,g,dom,idom,children,options):build()
    optimizeSSA(ssa)
    local regions=regionPlan(g,pdom,ipdom,loops)
    local st=ProductionStructurer.new(p,g,dom,idom,pdom,ipdom,loops,ssa,options)
    local body=cleanupSource(st:recover())
    local names=st:names(); local params={}
    for r=0,(p.numparams or 0)-1 do params[#params+1]=names[r] end
    if p.vararg then params[#params+1]="..." end
    local fname=identifier(p.debugname) and p.debugname or "anonymous"
    local source="function "..fname.."("..table.concat(params,", ")..")\n"..body.."\nend"
    local diagnostics=productionDiagnostics(p,g,ssa,regions,st)
    local confidence=productionConfidence(g,ssa,regions,st)
    return {source=source,analysis=metrics(p),ssaSummary={values=ssa.nextValue,phis=0,loops=#loops,blocks=#g.blocks,iterations=1},
        graph=valueGraph(ssa),confidence=confidence,diagnostics=diagnostics,regions=regions,
        dominators=dom,idom=idom,dominatorChildren=children,postdominators=pdom,ipdom=ipdom,exits=exits,
        warnings=diagnostics.warnings,chunk=options.returnChunk and chunk or nil,proto=p}
end

local function productionDecompile(input,options)
    options=options or {}
    local data=input
    if type(input)~="string" then
        local getter=options.getscriptbytecode or getscriptbytecode
    ...
    local c=parseAny(data,options)
    assert(c.main,"no main prototype found")
    local result=productionOne(c.main,options,c)
    result.format=c.format; result.bytecodeVersion=c.bytecodeVersion; result.typeVersion=c.typeVersion
    result.bytecodeSize=#data; result.protoCount=#(c.protos or {}); result.mainProto=c.main.id
    result.parserWarnings=c.warnings or {}
    -- Decompile children independently.  This is intentionally kept separate
    -- from the parent's expression graph so closures can be rendered without
    -- contaminating parent register state.
    result.protos={}
    for _,p in ipairs(c.protos or {}) do
        local ok,child=pcall(productionOne,p,options,c)
        result.protos[p.id]=ok and child or {error=tostring(child),id=p.id}
    end
    if options.aiReview then
        local proposed=aiReview(result,options)
        if proposed then result.aiCandidate=proposed end
    end
    return result
end

m0pu.decompile=productionDecompile
m0pu.decompileProduction=productionDecompile
m0pu.analyzeProduction=function(input,options)
    local r=productionDecompile(input,options or {})
    return {format=r.format,bytecodeVersion=r.bytecodeVersion,typeVersion=r.typeVersion,
        bytes=r.bytecodeSize,protoCount=r.protoCount,analysis=r.analysis,confidence=r.confidence,diagnostics=r.diagnostics}
end
m0pu.getProductionPipeline=function()
    return {
        "parse", "normalize", "cfg", "dominators", "postdominators",
        "dominance-frontiers", "phi-placement", "ssa-renaming", "use-def",
        "constant-fold", "copy-propagation", "value-graph", "loop-analysis",
        "control-dependence", "region-recovery", "structured-emission",
